...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==='mine'){
      ...
      const oldPoints=Number(current.mining_points??0);const level=miningLevel(oldPoints);const earned=miningPower(level);const newPoints=oldPoints+earned;const newLevel=miningLevel(newPoints);
    ...
    if(action==='daily_login'){
      ...
      const yesterday=new Date(Date.now()-86400000).toISOString().slice(0,10);const {data:prev}=await supabase.from('daily_logins').select('streak_value').eq('user_id',user.id).eq('login_date',yesterday).maybeSingle();const streak=prev?Number(prev.streak_value)+1:1;const reward=Math.min(0.01+streak*0.001,0.02);const {error:loginErr}=await supabase.from('daily_logins').insert({user_id:user.id,login_date:today,streak_value:streak,reward});if(loginErr)throw loginErr;const {data:wallet}=await supabase.from('wallets').select('*').eq('user_id',user.id).single();const newBalance=Number(wallet.balance)+reward;await supabase.from('wallets').update({balance:newBalance,lifetime_earned:Number(wallet.lifetime_earned)+reward,updated_at:new Date().toISOString()}).eq('user_id',user.id);await supabase.from('transactions').insert({user_id:user.id,type:'daily_login',amount:reward,balance_after:newBalance,description:`Daily login streak ${streak}`});await supabase.from('users').update({daily_streak:streak,last_login_at:new Date().toISOString()}).eq('id',user.id);return Response.json({ok:true,already_claimed:false,streak,reward,...(await balance(user.id))},{headers:cors});
    }

    if(action==='reward'){const adType=String(body.adType??'');const allowed=new Set(['rewarded_interstitial','rewarded_popup']);if(!allowed.has(adType))throw new Error('Unsupported ad type');const since=new Date(Date.now()-60000).toISOString();const {count}=await supabase.from('transactions').select('*',{count:'exact',head:true}).eq('user_id',user.id).eq('type','ad').gte('created_at',since);if((count??0)>=1)throw new Error('Ad reward cooldown active');const points=1;const amount=0.0001;const {data:wallet}=await supabase.from('wallets').select('*').eq('user_id',user.id).single();const newBalance=Number(wallet.balance)+amount;await supabase.from('wallets').update({balance:newBalance,lifetime_earned:Number(wallet.lifetime_earned)+amount,updated_at:new Date().toISOString()}).eq('user_id',user.id);await supabase.from('transactions').insert({user_id:user.id,type:'ad',amount,balance_after:newBalance,description:adType});return Response.json({ok:true,coins:newBalance,points},{headers:cors});}
    ...
    if(action==='withdraw'){const method=String(body.method??'');const accountValue=String(body.accountValue??'').trim();const amount=Number(body.amount??0);const minimums:Record<string,number>={bkash:1,usdt_bep20:5};if(!(method in minimums))throw new Error('Unsupported payout method');if(!accountValue||!Number.isFinite(amount)||amount<minimums[method])throw new Error(`Minimum withdrawal is ${minimums[method]}`);const {data:wallet}=await supabase.from('wallets').select('*').eq('user_id',user.id).single();if(Number(wallet.balance)<amount)throw new Error('Insufficient balance');const {data,error}=await supabase.from('withdrawals').insert({user_id:user.id,method,account_value:accountValue,amount}).select().single();if(error)throw error;return Response.json({ok:true,withdrawal:data},{headers:cors});}
